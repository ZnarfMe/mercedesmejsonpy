...
def _make_authorization_headers(client_id, client_secret):
    auth_header = base64.b64encode('{}:{}'.format(client_id, client_secret).encode('utf-8'))
...
class MercedesMeClientCredentials(object):
    ...
    def __init__(self, client_id, client_secret, redirect_uri):
        ...
        if not client_secret:
            raise MercedesMeOauthError('No client secret')

        if not redirect_uri:
            raise MercedesMeOauthError('No redirect_uri')
